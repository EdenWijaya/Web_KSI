...
const ArtikelTable = () => {
  ...
  const [artikelsPerPage] = useState(5); // Jumlah artikel per halaman
  ...
  return (
    <div className="bg-gray-100 min-h-screen py-1" style={{ marginLeft: "-20px", marginRight: "-20px" }}>
      ...
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-green-800 font-['Poppins'] underline">Tabel Artikel</h2>
      </div>

      {/* Tabel Desktop */}
      <div className="hidden md:block bg-white shadow-lg mx-auto w-full overflow-x-auto">
          ...
          <tbody>
            {currentArticles.map((artikel, index) => (
              <tr key={artikel.id_artikel} className={index % 2 === 0 ? "bg-gray-50" : "bg-white"}>
                ...
                <td className="px-2 py-2 text-center border-b break-words max-w-[150px]">{artikel.judul_artikel}</td>
                <td className="px-2 py-2 text-center border-b break-words max-w-[100px]">{artikel.author}</td>
                <td className="px-2 py-2 text-center border-b break-words max-w-[100px]">{artikel.sumber_artikel}</td>
                ...
                <td className="px-2 py-2 border-b">
                  {artikel.gambar_artikel ? (
                    <div className="flex justify-center items-center">
                      <img src={showGambar(artikel.gambar_artikel)} alt="Gambar Artikel" className="w-12 h-12 rounded" />
                    </div>
                  ) : (
        ...
        {currentArticles.map((artikel) => (
          <div key={artikel.id_artikel} className="bg-white shadow-lg rounded-lg p-4 max-w-sm mx-auto">
            <div className="grid grid-cols-3 gap-y-1 gap-x-2">
              ...
              <span className="col-span-2 break-words">: {artikel.isi_artikel.slice(0, 100)}...</span>
            </div>
            ...
            <div className="mt-4">
              ...
              {artikel.gambar_artikel ? (
                <div className="mt-2 flex justify-center">
                  <img src={showGambar(artikel.gambar_artikel)} alt="Gambar Artikel" className="w-full max-w-xs h-auto rounded-lg object-cover border border-gray-300 shadow-md" />
                </div>
              ) : (
                <span className="text-gray-500">: Tidak ada gambar</span>
              )}
            ...
            <div className="mt-4 flex justify-end space-x-2">
              <button onClick={() => navigate(`/update-artikel/${artikel.id_artikel}`)} className="px-4 py-2 text-sm font-semibold text-white bg-blue-600 rounded-lg hover:bg-blue-700 shadow-md">
                Edit
              </button>
              <button onClick={() => handleDelete(artikel.id_artikel)} className="px-4 py-2 text-sm font-semibold text-white bg-red-600 rounded-lg hover:bg-red-700 shadow-md">
                Hapus
              </button>
            </div>
          </div>
